...
def closest_obstacle_point(current_eef_pos, obstacle_points, *, xy_only=True):
    ...
    if points.ndim != 2 or points.shape[-1] != 3 or points.shape[0] == 0:
        raise ValueError("obstacle_points must have non-empty shape [N, 3]")
    if current.shape != (3,):
        raise ValueError("current_eef_pos must have shape [3]")
    dimensions = 2 if xy_only else 3
...
def paper_obstacle_cost(
    ...
    *,
    ...
    if raw_action.shape[-1] < 3:
        raise ValueError("Actions must contain XYZ in their first three coordinates")
    dimensions = 2 if xy_only else 3
    ...
    if point.shape != (3,):
        raise ValueError("obstacle_point must have shape [3]")
    delta = raw_action[..., :dimensions] - point[:dimensions]
