...
def set_obstacle_guidance_context(policy, env, obs, args, step_i=0, cached_pc_fields=None):
    algo = getattr(policy, "policy", policy)
    if not hasattr(algo, "set_obstacle_guidance_context"):
        raise RuntimeError(
            "Policy does not support obstacle guidance. "
            "Wrap it with wrap_as_guided(policy) first."
        )
    ...
    actions_unnormalized = (
        action_scale is not None
        and np.allclose(action_scale.flatten()[:3], 1.0, atol=1e-6)
        and np.allclose(action_offset.flatten()[:3], 0.0, atol=1e-6)
    )
    if actions_unnormalized:
        delta_pos_scale = np.ones_like(delta_pos_scale, dtype=np.float32)
        delta_pos_offset = np.zeros_like(delta_pos_offset, dtype=np.float32)
    ...
    context["diagnostic_oracle"] = dict(
        centers=np.asarray(diagnostic_centers, dtype=np.float32) if len(diagnostic_centers) > 0 else np.zeros((0, 3), dtype=np.float32),
        radii=np.asarray(diagnostic_radii, dtype=np.float32) if len(diagnostic_radii) > 0 else np.zeros((0,), dtype=np.float32),
        top_z=diagnostic_top_z if len(diagnostic_top_z) > 0 else np.zeros((0,), dtype=np.float32),
    )
    ...
    if args.guidance_geometry_source == "oracle_center":
        ...
        if cached_pc_fields is not None:
            pc_fields = cached_pc_fields
        else:
...
def rollout(policy, env, horizon, args, video_writer=None):
    ...
    pc_cache = None
    pc_cache_dirty = True
    use_pc_cache = getattr(args, "no_pc_cache", False) is False

    for step_i in range(horizon):
        if args.guidance_geometry_source == "pointcloud" and (pc_cache_dirty or not use_pc_cache):
            pc_cache = build_pointcloud_context_fields(env=env, obs=obs, args=args, step_i=step_i)
            if use_pc_cache:
                pc_cache_dirty = False
        ...
        obstacle_info = set_obstacle_guidance_context(
            ...
            args=args,
            step_i=step_i,
            cached_pc_fields=pc_cache if (args.guidance_geometry_source == "pointcloud" and use_pc_cache) else None,
        )
        ...
        if args.guidance_geometry_source == "pointcloud" and pc_cache is not None and use_pc_cache:
            next_eef_pos = get_current_eef_pos_from_obs(obs=next_obs, obs_key=args.eef_pos_obs_key)
            pts = pc_cache["obstacle_points_world"]
            if len(pts) > 0:
                min_dist = float(np.min(np.linalg.norm(pts[:, :2] - next_eef_pos[:2], axis=1)))
                if min_dist < 0.02:
                    pc_cache_dirty = True
...
def parse_args():
    ...
    parser.add_argument(
        "--guidance_start_step_pct",
        type=float,
        default=0.7,
        help="fraction of denoising steps before guidance activates (0-1, default 0.7 skips first 70%%)",
    )
    ...
    parser.add_argument("--no-pc-cache", action="store_true", help="disable static pointcloud caching (recompute every step)")
